...
export default async function FuzzProposalPage({ params }) {
  ...
  const config = PROPOSAL_TYPES.fuzzing || PROPOSAL_TYPES.smart_contract
    ...
    <>
      ...
      <div className="cover">
        <div className="wrap">
          ...
          <div className="eyebrow">Fuzz Testing (FaaS)</div>
          <h1 className="cover-title">
            Smart Contract<br /><span className="accent">Fuzz Testing</span>
          </h1>
          <p className="cover-sub">
            {`Prepared exclusively for ${proposal.client_name} at ${proposal.company}. Adversarial input generation at scale, beyond what static analysis can find.`}
          </p>
          ...
          <div className="cover-badge">
            ...
            Only 2 fuzz campaign slots remaining. Availability is limited
          </div>
        </div>
      </div>
      ...
      <div className="section page-break">
        <div className="wrap">
          ...
          <h2 className="section-title">Beyond static analysis.<br /><span className="accent">Adversarial input at scale.</span></h2>
          <p className="section-sub">Traditional audits review code paths humans can see. Fuzz testing generates millions of randomised inputs to find what no one thought to look for.</p>
          <div className="ov-grid">
            {[
              { icon: '🔬', title: 'Stateful Invariant Testing',  desc: 'Multi-step transaction sequences that test protocol-wide invariants after every state change.' },
              { icon: '⚡', title: 'Property-Based Fuzzing',      desc: 'Randomised inputs with postcondition assertions. Millions of cases per function.' },
              { icon: '💰', title: 'Economic Exploit Detection',   desc: 'Flash loan, MEV, and tokenomics manipulation scenarios that static analysis misses.' },
              { icon: '🧬', title: 'Reproducer Extraction',        desc: 'Every failing case minimised to its smallest seed with full execution traces.' },
            ].map(c => (
      ...
      <div className="section page-break">
        <div className="wrap">
          <div className="section-label">02 Scope & Pricing</div>
          <h2 className="section-title">Your Fuzz Campaign, <span className="accent">Scoped & Priced</span></h2>
          <p className="section-sub">Everything included. No surprises. 50% to start, 50% after delivery.</p>
          <div className="urgency">
            <span>⚠️</span>
            <span><strong style={{ color: '#ff4f6a' }}>$3.6B+ was lost to exploits in 2025.</strong> A fuzz campaign at this stage is the highest-ROI security investment your protocol will ever make.</span>
          </div>
          <div className="price-card avoid-break">
            <div className="price-head">
              <span className="price-head-txt">{proposal.company} | Fuzz Testing (FaaS)</span>
            ...
            <div className="price-footer">
              {['50% upfront, 50% on delivery', 'USDC / Fiat accepted', 'Full fuzz seeds included', 'Free retest window'].map(t => <span key={t}>{t}</span>)}
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="section page-break">
        <div className="wrap">
          <div className="section-label">03 Testing Coverage</div>
          <h2 className="section-title">{vulnRows.length} Testing Modules. <span className="accent">Mapped to Standards</span></h2>
          <p className="section-sub">Every module we test, with the specific checkpoints our fuzz harnesses verify.</p>
          <table className="vuln-table">
            <thead><tr><th className="vuln-th">Testing Module</th><th className="vuln-th">Checkpoints</th></tr></thead>
            <tbody>
              {vulnRows.map((row, i) => (
      ...
      <div className="section page-break">
        <div className="wrap">
          ...
          <p className="section-sub">Three compounding credibility assets, not just a PDF.</p>
          <div className="del-grid">
            {[
              { icon: '📋', title: 'Full Fuzz Report',         desc: 'Technical findings with severity ratings, minimal reproducers, seeds, and gas profiling data.', tag: 'Engineer-ready' },
              { icon: '🧬', title: 'Fuzz Harness Suite',       desc: 'All custom Foundry/Echidna harnesses delivered as reusable test infrastructure for your team.', tag: 'Reusable' },
              { icon: '🛡️', title: 'Security Badge & Seal',   desc: 'Display it on your site, pitch deck, and docs. Instant trust signal for users and investors.', tag: 'Display anywhere' },
            ].map(d => (
          ...
          <table className="comp-table">
            <thead>
              <tr>
                ...
                <th className="comp-th-us">CredShields FaaS ✦</th>
            ...
            <tbody>
              {[
                ...
                { label: 'Harness delivery',us: '✓ Included',                     vals: compRows.map(() => '✗') },
                { label: 'Full seeds',     us: '✓ Included',                      vals: compRows.map(() => '✗') },
      ...
      <div className="cta">
        <div className="wrap">
          <h2 className="cta-title">
            Your security. Your users.<br /><span className="accent">Your reputation.</span>
          </h2>
          <p className="cta-sub">
