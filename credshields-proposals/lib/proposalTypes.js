...
// ── Vulnerability coverage tables ─────────────────────────────────────────
export const VULNERABILITIES = {
  smart_contract: {
    title: 'Vulnerabilities in Smart Contracts',
    colLabel: 'CHECKPOINTS',
    rows: [
      { vuln:'Access Control Flaws',           checks:['Misconfigured ownership and roles','Improper use of onlyOwner or require checks','Privilege escalation vulnerabilities','Validate security posture holistically'] },
      { vuln:'Reentrancy Risks',               checks:['Checks for reentrancy in call(), delegatecall(), transfer() functions','Use of mutex-like modifiers','Cross-function race condition reviews'] },
      { vuln:'Integer Overflow / Underflow',   checks:['Arithmetic precision issues in add, sub, mul, div','Improper handling of signed/unsigned integers'] },
      { vuln:'Oracle & Price Feed Manipulation',checks:['Susceptibility to manipulated price feeds','Lack of fallback or time-weighted average price (TWAP) logic'] },
      { vuln:'Business Logic Errors',          checks:['Flaws in lending/borrowing/tokenomics mechanisms','Inconsistent reward or minting logic','Functionality deviations from spec'] },
      { vuln:'Lack of Input Validation',       checks:['Absence of require()/assertions for critical parameters','Susceptibility to invalid/unexpected user inputs'] },
      { vuln:'Flash Loan Exploits',            checks:['Protocol behavior under multi-step flash execution','Temporal access inconsistencies','Asset draining opportunities'] },
      { vuln:'DoS & Gas Griefing',             checks:['Expensive loop executions','Unbounded array/storage growth','Front-running and block stuffing edge cases'] },
      { vuln:'Insecure Randomness',            checks:['Reliance on block variables for entropy (e.g., block.timestamp, block.number)','No use of secure randomness sources (e.g., Chainlink VRF)'] },
      { vuln:'Low-level Calls & Delegation',   checks:['Improper usage of delegatecall or callcode','Potential code injection and delegate context abuse'] },
      { vuln:'Contract Upgradeability',        checks:['Misuse of proxy patterns','Unchecked upgrade permissions','Uninitialized contracts or logic contracts'] },
      { vuln:'Economic Attack Surface',        checks:['Sandwich attacks','MEV-prone functions','Token pricing manipulation across DEXs'] },
      { vuln:'Gas Optimization Issues',        checks:['Unused storage variables','Inefficient state variable usage','Redundant computations on-chain'] },
    ],
  },
  web_app: {
    ...
    colLabel: 'CHECKPOINTS',
    rows: [
      { vuln:'Access Control Flaws',           checks:['Broken access control (A01:2021)','Insecure direct object references (IDOR)','Missing or bypassable authorization checks'] },
      { vuln:'Authentication Issues',          checks:['Broken authentication (A07:2021)','Predictable or exposed session identifiers','Missing MFA implementation','Brute-force vulnerabilities'] },
      { vuln:'Session Management',             checks:['Session fixation','Insecure cookie flags (Secure, HttpOnly, SameSite)','Session timeout misconfigurations','Token leakage via referrer or logs'] },
      { vuln:'Input Validation & Injection',   checks:['Injection vulnerabilities (A03:2021): SQLi, NoSQLi, Command Injection','Cross-site scripting (XSS)','XML External Entities (XXE)','Server-side template injection'] },
      { vuln:'Cryptographic Failures',         checks:['Use of deprecated algorithms (e.g., MD5, SHA-1)','Weak TLS/SSL configurations','Missing encryption for sensitive data in transit or at rest'] },
      { vuln:'Security Misconfigurations',     checks:['Default credentials or admin panels exposed','Missing security headers (CSP, HSTS, X-Content-Type-Options, etc.)','Verbose error handling'] },
      { vuln:'Vulnerable Components',          checks:['Known vulnerable libraries and packages (A06:2021)','Outdated third-party dependencies','Lack of software composition analysis (SCA)'] },
      { vuln:'Insecure Design & Logic Flaws',  checks:['Business logic vulnerabilities','Lack of input validation for expected workflows','Abuse of trust boundaries or assumptions'] },
      { vuln:'API Security Flaws',             checks:['Broken object-level authorization (BOLA)','Excessive data exposure in APIs','Lack of rate limiting or throttling','Improper HTTP methods handling'] },
      { vuln:'Data Exposure Risks',            checks:['Sensitive information in URLs, responses, or logs','Client-side storage of sensitive data (localStorage/sessionStorage)','Verbose API responses'] },
      { vuln:'Rate Limiting & DoS Vectors',    checks:['Missing or weak rate limiting on critical endpoints','Abuse of resource-heavy operations','Unbounded file uploads or loops'] },
      { vuln:'File Upload & Parsing Issues',   checks:['Lack of MIME-type and file extension validation','Upload of executable files','Insecure deserialization or file parsing logic'] },
      { vuln:'CSRF & State Manipulation',      checks:['Missing anti-CSRF tokens on state-changing requests','Predictable or reusable tokens','Exploitable state transitions'] },
      { vuln:'Client-Side Security Flaws',     checks:['DOM-based XSS','Open redirect vulnerabilities','Insecure use of eval() and similar JS functions','CORS misconfigurations'] },
    ],
  },
}
